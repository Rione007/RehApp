...
class RehabFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        recyclerRehab.adapter = RehabAdapter(rehabs) { selectedRehab ->
            guardarRehabSeleccionada(selectedRehab)
    ...
    private fun guardarRehabSeleccionada(rehab: Rehab) {
        val prefs = requireContext().getSharedPreferences("rehapp_user", Context.MODE_PRIVATE)
        val editor = prefs.edit()
        editor.putInt("rehab_id", rehab.id)
        editor.putString("rehab_nombre", rehab.name)
        editor.apply()

        println("💾 Rehabilitación guardada: ${rehab.name}")
    }
    ...
    companion object {
        fun newInstance(): RehabFragment = RehabFragment()
    }
}
